...
export const get_attention_metrics = (events: Event[] = []) => {
  ...
  const sats_per_second_max = Math.max(...attention_events.map((event: Event) => parseInt(event.tags.find((tag) => tag[0] === "sats_per_second")?.[1] || "0")));
  const sats_per_second_min = Math.min(...attention_events.map((event: Event) => parseInt(event.tags.find((tag) => tag[0] === "sats_per_second")?.[1] || "0")));
