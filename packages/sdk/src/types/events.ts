...
/**
 * MARKETPLACE Event (kind 38088) parameters
 */
export interface MarketplaceEventParams extends BaseEventParams {
  ...
  owner_pubkey: string; // Marketplace owner pubkey (for p tag)
...
/**
 * BILLBOARD Event (kind 38188) parameters
 */
export interface BillboardEventParams extends BaseEventParams {
  ...
  marketplace_coordinate: string; // Marketplace coordinate: 38088:<marketplace_pubkey>:<marketplace_id> (required)
...
/**
 * PROMOTION Event (kind 38288) parameters
 */
export interface PromotionEventParams extends BaseEventParams {
  ...
  billboard_coordinate: string; // Billboard coordinate: 38188:<billboard_pubkey>:<billboard_id> (required)
  marketplace_pubkey: string; // Marketplace pubkey (required)
...
/**
 * ATTENTION Event (kind 38388) parameters
 */
export interface AttentionEventParams extends BaseEventParams {
  ...
  marketplace_coordinate: string; // Marketplace coordinate: 38088:<marketplace_pubkey>:<marketplace_id> (required)
  block_list_coordinate: string; // Block list coordinate: 38988:<block_list_owner_pubkey>:<block_list_d_tag> (required)
...
export interface MatchEventParams extends BaseEventParams {
  ...
  marketplace_coordinate: string; // a tag: 38088:<marketplace_pubkey>:<marketplace_id>
  billboard_coordinate: string; // a tag: 38188:<billboard_pubkey>:<billboard_id>
  promotion_coordinate: string; // a tag: 38288:<promotion_pubkey>:<promotion_id>
  attention_coordinate: string; // a tag: 38388:<attention_pubkey>:<attention_id>
  ...
  kind: number | number[]; // Event kind(s) for k tag (multiple allowed)
}

/**
 * BILLBOARD_CONFIRMATION Event (kind 38488) parameters
 */
...
/**
 * VIEWER_CONFIRMATION Event (kind 38588) parameters
 */
...
/**
 * MARKETPLACE_CONFIRMATION Event (kind 38688) parameters
 */
