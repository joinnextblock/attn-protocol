...
NIP-X1 defines the core PROMO Protocol for content promotion on Nostr, establishing standardized event kinds and communication patterns between participants. The protocol establishes a market-driven system connecting PROMOTION Creators who wish to promote content with ATTENTION Owners who are willing to view it, facilitated by BILLBOARD OPERATORS. This foundation enables a transparent, decentralized alternative to traditional advertising systems while preserving Nostr's core principles of user sovereignty and decentralization.
...
- **ATTENTION Owners**: Nostr identity that publishes ATTENTION events
- **BILLBOARD Operator**: Nostr identity that PROMOTION Creators and ATTENTION Owners signal they trust via trusted BILLBOAD list []()
...
- `sats_per_second`: ATTENTION Owner requested payment rate in sats per second
- `k`: Event kinds the ATTENTION Owner is willing to watch
...
```json
{
  ...
  "tags": [
    ...
    ["attention_pubkey", "<ATTENTION_OWNER_pubkey>"],
    // MATCH
    ["a", "kind:<32-bytes lowercase hex of a pubkey>:<d tag value>"],
    ["p", "<BROKERAGE_pubkey>"],
...
```json
{
  ...
  "tags": [
    ...
    ["promotion_acceptance_pubkey", "<ATTENTION_OWNER_pubkey>"],
  ]
}
```
...
{
  "kind": 30000,
  "pubkey": "<ATTENTION_OWNER_pubkey | PROMOTION_CREATOR_pubkey | BROKERAGE_pubkey> ",
  "created_at": <unix_timestamp>,
  "tags": [
...
{
  "kind": 30003,
  "pubkey": "<ATTENTION_OWNER_pubkey | BROKERAGE_pubkey> ",
  "created_at": <unix_timestamp>,
  "tags": [
...
- Trust is established when PROMOTION Creators and ATTENTION Owners include a BILLBOARD_pubkey in trusted BILLBOARD list
...
3. **ATTENTION**: ATTENTION Owners `write` ATTENTION(kind:38888) event(s) to RELAY LIST(kind:10002)
4. **MATCH**: BROKERAGE OPERATOR `write` MATCH(kind:38388) event(s) to RELAY LIST(kind:10002)
5. **DISPLAY**: BILLBOARD displays promoted EVENT in PROMOTION event to ATTENTION Owner based on 'best' MATCH event
